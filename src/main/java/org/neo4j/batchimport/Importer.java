...
public class Importer {
	private static final Map<String, String> SPATIAL_CONFIG = Collections
			.singletonMap(IndexManager.PROVIDER, "spatial");
	...
	public Importer(File graphDb, final Config config) {
		...
		if (indexInfos != null) {
			for (IndexInfo indexInfo : indexInfos) {
				BatchInserterIndex index = indexInfo.isNodeIndex() ? nodeIndexFor(
						indexInfo.indexName, indexInfo.indexType)
						: relationshipIndexFor(indexInfo.indexName,
								indexInfo.indexType);
	...
	protected BatchInserterIndexProvider createIndexProvider(
		...
		return luceneOnlyIndex ? new LuceneBatchInserterIndexProvider(db)
				: new MapDbCachingIndexProvider(db);
	}

	protected BatchInserter createBatchInserter(File graphDb, Config config) {
		return BatchInserters.inserter(graphDb.getAbsolutePath(),
	...
	public static void main(String... args) throws IOException {
		System.err
				.println("Usage: Importer data/dir nodes.csv relationships.csv [node_index node-index-name fulltext|exact nodes_index.csv rel_index rel-index-name fulltext|exact rels_index.csv ....]");
	...
	void importNodes(Reader reader) throws IOException {
		...
		while (data.processLine(null)) {
			...
			for (Map.Entry<String, Map<String, Object>> entry : data
					.getIndexData().entrySet()) {
				final BatchInserterIndex index = indexFor(entry.getKey());
				if (index == null)
					throw new IllegalStateException("Index " + entry.getKey()
							+ " not configured.");
				index.add(id, entry.getValue());
			}
			report.dots();

			if (report.getCount() % BATCH == 0)
				flushIndexes();
		}
		flushIndexes();
	...
	private Label[] labelsFor(String[] labels) {
		...
		for (int i = labels.length - 1; i >= 0; i--) {
			if (labelsArray[i] == null
					|| !labelsArray[i].name().equals(labels[i]))
	...
	void importRelationships(Reader reader) throws IOException {
		...
		while (data.processLine(null)) {
			...
			final RelType type = relType
					.update(data.getRelationshipTypeLabel());
	...
	private LineData createLineData(Reader reader, int offset) {
		...
		if (useQuotes) {
			return new CsvLineData(reader, config.getDelimChar(this), offset,
					config);
		} else {
			return new ChunkerLineData(reader, config.getDelimChar(this),
					offset, config);
		}
	}
	...
	private void importIndex(IndexInfo indexInfo) throws IOException {
		...
		importIndex(indexInfo.indexName, indexes.get(indexInfo.indexName),
				createFileReader(indexFile));
	}
