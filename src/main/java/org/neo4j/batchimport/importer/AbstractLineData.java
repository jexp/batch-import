...
public abstract class AbstractLineData implements LineData {
	...
	public AbstractLineData(int offset, Config config) {
		this.offset = offset;
	...
	public String[] getTypeLabels() {
		...
		return labels instanceof String ? new String[] { labels.toString() }
				: (String[]) labels;
	}
	...
	public String getRelationshipTypeLabel() {
		...
		return labels instanceof String[] ? ((String[]) labels)[0]
				: (String) labels;
	}
	...
	protected Object convert(int column, String value) {
		try {
			return headers[column].type == Type.STRING ? value
					: headers[column].type.convert(config, value);
		} catch (Exception e) {
			// todo potentially skip?
			throw new RuntimeException("Error converting value row " + rows
					+ " column " + headers[column] + " value " + value
