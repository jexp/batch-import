...
public class RowData implements LineData {
	...
	public RowData(String header, String delim, int offset, Config config) {
		this.offset = offset;
		this.delim = delim;
	...
	private Header[] createHeaders(String[] fields) {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (type == Type.LABEL
					|| name.toLowerCase()
							.matches("^(type|types|label|labels)$")) {
				labelId = i;
				type = Type.LABEL;
				explicitLabelId = i;
			}
	...
	public Object getValue(int column) {
		return getHeader(column).type.convert(config, lineData[column]);
	}
	...
	private int parse(String line) {
		...
		for (int i = 0; i < lineSize; i++) {
			...
			if (value.equals(delim)) {
				...
				if (i < lineSize - 1 && st.hasMoreTokens())
					st.nextToken();
			}
		}
