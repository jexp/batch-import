...
public class Config {
	...
	private String arraysSeparator = ",";

	/**
	 * @return the arraysSeparator
	 */
	public String getArraysSeparator() {
		return arraysSeparator;
	}

	/**
	 * @param c
	 *            the arraysSeparator to set
	 */
	public void setArraysSeparator(String c) {
		this.arraysSeparator = c;
	}
	...
	public Config(Map<String, String> configData) {
		...
		if (configData != null
				&& this.configData.containsKey(ARRAY_SEPARATOR_CONFIG)) {
			setArraysSeparator(configData.get(ARRAY_SEPARATOR_CONFIG));
		}else if(configData==null){
			logger.warn("A null configuration was created.");
		}
	}
	...
	public static Config convertArgumentsToConfig(String[] args) {
		...
		final Map<String, String> config = configFromFile(configFileName);
	...
	private static void validateConfig(Map<String, String> config) {
		if (!config.containsKey(BATCH_IMPORT_GRAPH_DB))
			throw new IllegalArgumentException(
					"Missing parameter for graphdb directory");
	}

	private static Collection<IndexInfo> convertParamsToConfig(
			Stack<String> args, Map<String, String> config) {
	...
	private static void addConfigParamIfArgument(Stack<String> args,
			Map<String, String> config, String param) {
	...
	private static Collection<IndexInfo> createIndexInfos(Stack<String> args) {
		...
		while (!args.isEmpty()) {
			indexes.add(new IndexInfo(popOrNull(args), popOrNull(args),
					popOrNull(args), popOrNull(args)));
		}
		return indexes;
	}

	public static Map<String, String> configFromFile(String fileName) {
		...
		try {
			if (new File(fileName).exists()) {
				...
				System.out
						.println("Writing Configuration File to batch.properties");
	...
	public static Collection<IndexInfo> extractIndexInfos(
		...
		for (String part : commaSeparatedFileList.split(",")) {
			...
			if (file.exists() && file.canRead() && file.isFile())
				...
				System.err.println("File " + file
						+ " does not exist, can not be read or is not a file.");
		}
		return files;
	}
