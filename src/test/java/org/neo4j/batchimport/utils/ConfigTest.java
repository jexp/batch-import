...
public class ConfigTest {
	...
	private final Config c = new Config(null);

	private File createTempFile(String prefix, String suffix) {
		final File tempFile;
		try {
			tempFile = File.createTempFile(prefix, "." + suffix, new File(
					"target"));
	...
	// final String[] args =
	// "data/dir nodes.csv relationships.csv [node_index node-index-name fulltext|exact nodes_index.csv rel_index rel-index-name fulltext|exact rels_index.csv ....]".split(" ");

	@Test
	public void testExtractDatabaseDir() throws Exception {
		assertCommandLine("data/dir", "data/dir", Config.BATCH_IMPORT_GRAPH_DB);
	}

	@Test
	public void testToFiles() throws Exception {
		final Collection<File> files = Config.toFiles("null,,foo,"
				+ nodesFile.getAbsolutePath());
		assertEquals(1, files.size());
		assertEquals(nodesFile.getAbsolutePath(), files.iterator().next()
				.getAbsolutePath());
	...
	public void testExtractRelsFiles() throws Exception {
		assertCommandLine("data/dir " + nodesFile.getAbsolutePath() + " "
				+ relsFile.getAbsolutePath(), relsFile.getAbsolutePath(),
				Config.BATCH_IMPORT_RELS_FILES);
	}

	@Test
	public void testExtractExactNodeIndexFile() throws Exception {
		assertCommandLine(
				"data/dir nodes.csv rels.csv node_index index-name exact node_index.csv",
	...
	public void testExtractFulltextNodeIndexFile() throws Exception {
		assertCommandLine(
				"data/dir nodes.csv rels.csv node_index index-name fulltext node_index.csv",
	...
	public void testExtractExactNodeIndex() throws Exception {
		assertCommandLine(
				"data/dir nodes.csv rels.csv node_index index-name exact",
	...
	public void testExtractFulltextNodeIndex() throws Exception {
		assertCommandLine(
				"data/dir nodes.csv rels.csv node_index index-name fulltext",
	...
	public void testCustomArraySeparator() throws Exception {
		...
		c.setArraysSeparator(",");
	}

	@Test
	public void testExtractExactRelsIndexFile() throws Exception {
		assertCommandLine(
				"data/dir nodes.csv rels.csv relationship_index index-name exact rels_index.csv",
	...
	public void testExtractExactRelsIndex() throws Exception {
		assertCommandLine(
				"data/dir nodes.csv rels.csv relationship_index index-name exact",
	...
	private void assertCommandLine(String arguments, String expected,
			String optionName) {
