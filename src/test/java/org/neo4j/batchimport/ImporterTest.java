...
public class ImporterTest {
	...
	public void setUp() throws Exception {
		...
		final Map<String, String> configData = Config
				.configFromFile("batch.properties");
		config = new Config(configData);
		new IndexInfo("node_index", "index-a", "exact", null)
	...
	public void testImportHashes() throws Exception {
		importer.importNodes(new StringReader(
		...
		importer.finish();
		verify(inserter, times(1)).createNode(
				eq(map("a", "000000F8BE951D6DE6480F4AFDFB670C553E47C0")));
		verify(inserter, times(1)).createNode(
				eq(map("a", "0000021449360C1A398ED9A18800B2B13AA098A4")));
		verify(inserter, times(1)).createNode(
				eq(map("a", "00000DABDE4C555FC82F7D534835247B94873C2C")));
		verify(inserter, times(1)).createNode(
				eq(map("a", "00001BE4128DB41729365A41D3AC1D019E5ED8A6")));
	}
	...
	public void testImportNodeWithTwoLabels() throws Exception {
		...
		importer.finish();
		verify(inserter, times(1)).createNode(eq(map("a", "foo")),
				eq(DynamicLabel.label("bar")), eq(DynamicLabel.label("bor")));
	}
	...
	public void testImportRelWithIndexLookup() throws Exception {
		when(index.get("a", "foo")).thenReturn(
				new LongIterableIndexHits(asList(42L)));
		importer.importRelationships(new StringReader(
				"a:string:index-a\tb\tTYPE\nfoo\t123\tFOOBAR"));
		importer.finish();
		verify(index, times(1)).get(eq("a"), eq("foo"));
		verify(inserter, times(1)).createRelationship(eq(42L), eq(123L),
				Matchers.any(RelationshipType.class), eq(map()));
	}

	@Test
	public void testImportRelationshipsWithNonIndexedNodes() throws Exception {
		when(index.get("node", "a")).thenReturn(
				new LongIterableIndexHits(asList(1L)));
		when(index.get("node", "b")).thenReturn(
				new LongIterableIndexHits(Arrays.<Long> asList()));
		importer.importRelationships(new StringReader(
				"node:string:index-a\tnode:string:index-a\ttype\na\ta\tTYPE\na\tb\tTYPE\nb\ta\tTYPE"));
		importer.finish();
		verify(inserter, times(1)).createRelationship(eq(1L), eq(1L),
				argThat(new RelationshipMatcher("TYPE")), eq(map()));
		verify(inserter, never()).createRelationship(eq(1L), eq(-1L),
				argThat(new RelationshipMatcher("TYPE")), eq(map()));
		verify(inserter, never()).createRelationship(eq(-1L), eq(1L),
				argThat(new RelationshipMatcher("TYPE")), eq(map()));
	}
	...
	public void testImportNodeWithIndividualTypes() throws Exception {
		...
		importer.finish();
		verify(inserter, times(1)).createNode(
				eq(map("a", 10, "b", 10.0F, "c", 1E+10F)));
	}

	@Test
	public void testImportNodeWithArrayTypes() throws Exception {
		importer.importNodes(new StringReader(
				"a:STRING_ARRAY\tb:float\tc:float\n10,11,12\t10.0\t1E+10"));
		importer.finish();
	...
	public void testImportSimpleRelationship() throws Exception {
		importer.importRelationships(new StringReader(
				"start\tend\ttype\ta\n1\t2\tTYPE\tfoo"));
		importer.finish();
		verify(inserter, times(1)).createRelationship(eq(1L), eq(2L),
				argThat(new RelationshipMatcher("TYPE")), eq(map("a", "foo")));
	}
	...
	public void testImportRelationshipWithIndividualTypes() throws Exception {
		importer.importRelationships(new StringReader(
				"start\tend\ttype\ta:int\tb:float\tc:float\n1\t2\tTYPE\t10\t10.0\t1E+10"));
		importer.finish();
		verify(inserter, times(1)).createRelationship(eq(1L), eq(2L),
				argThat(new RelationshipMatcher("TYPE")),
				eq(map("a", 10, "b", 10.0F, "c", 1E+10F)));
	}
}
